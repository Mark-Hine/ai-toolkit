...
- SwiftFormat/SwiftLint on edit: this plugin's `codex/hooks/swift_lint.py` runs them after each `.swift` edit when the repo has `.swiftformat` or `.swiftlint.yml` and the binary is installed.
