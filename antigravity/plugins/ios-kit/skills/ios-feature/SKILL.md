...
3. **Consult references.** Load `/ios-standards` for house patterns (state ownership, one-shot events, design system).
...
4. **Plan.** Files to add or change, state model (one `enum State` per screen: `loading` / `loaded(...)` /
   `error(...)`), where data code goes, DI wiring, test list, simulator checks. Follow the project's architecture rules. Proceed within authorized scope; ask only about unresolved scope or consequential choices.
...
6. **Tests.** View-model and service tests in the project's framework (Swift Testing or XCTest per `AGENTS.md`; Quick/Nimble
   only where target already uses it). Run them with `xcodebuild test … -only-testing:<Target>/<Suite>` and quote the
