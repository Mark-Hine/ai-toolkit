...
- Framework per repo, named in its `CLAUDE.md`: Swift Testing (`@Test`, `#expect`, `#require`, `@Suite`) for new tests
...
- Run targeted: `xcodebuild test -workspace <ws> -scheme "<test scheme>" -destination '<sim>' -only-testing:<Target>/<Suite>`
  using the command form from the project's `CLAUDE.md`; summarise with `xcrun xcresulttool get test-results summary`.
