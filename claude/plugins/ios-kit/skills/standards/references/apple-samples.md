...
- **Design-system layer**: `ButtonStyle`, `LabelStyle` and `ViewModifier` conformances plus `@ViewBuilder` slots instead
  ...
  has a `#Preview` with fixture data. This is the design-system package's shape too (named in each repo's `CLAUDE.md`).
...
- Each sample targets the OS it launched with. `@Observable` needs iOS 17, Swift Testing needs Xcode 16, Liquid Glass
  needs iOS 26. Repos with a lower deployment target keep `ObservableObject`/`@StateObject` for those screens and say so in `CLAUDE.md`.
