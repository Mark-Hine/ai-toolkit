# Per-repo `CLAUDE.md` template for iOS repos

Copy to `<repo>/CLAUDE.md`, replace every `<…>`, delete lines that do not apply. Keep it under ~60 lines with the hard
rules in the first 40; repo facts only, no tutorials (the shared `ios-kit` skills and `claude-rules/ios/` carry conventions).
Personal or temporary notes go in `CLAUDE.local.md` (gitignored).
...
- Branches `feature|fix|task/<TICKET>-<slug>` off `<develop>`. Formal PR reviews: `pr-review-skill`.
